...
def main(year_):
	...
	# drop dt_last <= march and dt_first >= nov (consider deleting before march and after nov)
	# !!! Attention: changes the original dataset (perform only once)
	march_fires = fires_cp[fires_cp.dt_last.dt.month <= 3].index
	nov_fires = fires_cp[fires_cp.dt_first.dt.month >= 11].index
	print(march_fires, nov_fires, sep="\n")
	print(march_fires.shape[0], nov_fires.shape[0], fires_cp.shape[0])
	fires_cp.drop(march_fires, axis=0, inplace=True)
